...
export function projectionConfidenceSummary(modelQuality: ModelQuality): string {
  const { confidence_label, projection_tier } = modelQuality;
  const tierNote =
...
export function projectionConfidenceTooltip(modelQuality: ModelQuality): string {
  const { components, component_weights, round_simulations, target_simulations } =
    modelQuality;
  const weights = component_weights ?? {
  ...
  const lines = [
    ...
    `Sim depth (${Math.round(weights.simulation_depth * 100)}% weight): ${pct(components.simulation_factor)}${
      round_simulations && target_simulations
        ? ` — ${round_simulations.toLocaleString()} / ${target_simulations.toLocaleString()} runs`
        : ""
    }`,
